...
--------------------------
Proxy para la API interna de SofaScore vía ScraperAPI.
"""
...
def sofascore_get(endpoint: str):
    ...
    if SCRAPER_API_KEY:
        ...
        params = {"api_key": SCRAPER_API_KEY, "url": target_url, "render": "false"}
        try:
    ...
    for period_block in data.get("statistics", []):
        ...
        for group in period_block.get("groups", []):
            for item in group.get("statisticsItems", []):
                ...
                period_stats[key] = {"name": item.get("name"), "home": item.get("home"), "away": item.get("away")}
...
def match_lineups(match_id: int):
    ...
    def parse_lineup(team_data):
        ...
        for p in team_data.get("players", []):
            info = p.get("player", {})
            players.append({"name": info.get("name"), "position": p.get("position"), "jersey": p.get("jerseyNumber"), "substitute": p.get("substitute", False), "rating": p.get("statistics", {}).get("rating")})
        return {"formation": team_data.get("formation"), "players": players}
    return jsonify({"match_id": match_id, "home": parse_lineup(data.get("home", {})), "away": parse_lineup(data.get("away", {}))})
...
def team_form(team_id: int):
    ...
    return jsonify({"team_id": team_id, "summary": {"W": results.count("W"), "D": results.count("D"), "L": results.count("L"), "string": "".join(results)}, "matches": form})
...
def match_prematch(match_id: int):
    ...
    return jsonify({
        ...
        "tournament": event.get("tournament", {}).get("name"),
        "date_ts": event.get("startTimestamp"),
        "home_team": {"id": home_id, "name": event.get("homeTeam", {}).get("name")},
        "away_team": {"id": away_id, "name": event.get("awayTeam", {}).get("name")},
        "h2h": {"total": len(h2h), "matches": h2h},
        "home_form": {"summary": {"W": home_results.count("W"), "D": home_results.count("D"), "L": home_results.count("L"), "string": "".join(home_results)}, "matches": home_form},
        "away_form": {"summary": {"W": away_results.count("W"), "D": away_results.count("D"), "L": away_results.count("L"), "string": "".join(away_results)}, "matches": away_form},
    })
